    ...
    if (response.statusCode == 200) {
      ...
      if (data['drivers'] != null) {
        ...
        final matched = list.firstWhere(
          (e) => (e['id'] == driverId || e['_id'] == driverId || e['uid'] == driverId),
          orElse: () => null,
        );
        if (matched != null) {
          driverUserModel.value = DriverUserModel.fromJson(matched);
          log("==============> Driver Model (API): ${driverUserModel.value.toJson()}");
          return;
        }
      }
    }
  ...
  Future<void> approveDriver() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse("${ApiConstant.baseUrl}/admin/drivers/${driverUserModel.value.id}/approve"),
        headers: ApiConstant.headers(token: token),
      );
      if (response.statusCode == 200) {
        ShowToastDialog.toast("Driver approved successfully");
        await _getArgument();
      } else {
        ShowToastDialog.toast("Failed to approve driver");
      }
    } catch (e) {
      log("Error approving driver: $e");
      ShowToastDialog.toast("Error approving driver");
    } finally {
      isLoading.value = false;
    }
  }

  Future<void> suspendDriver() async {
    isLoading.value = true;
    try {
      String token = await AppSharedPreference.getString('adminToken');
      final response = await http.post(
        Uri.parse("${ApiConstant.baseUrl}/admin/drivers/${driverUserModel.value.id}/suspend"),
        headers: ApiConstant.headers(token: token),
      );
      if (response.statusCode == 200) {
        ShowToastDialog.toast("Driver suspended successfully");
        await _getArgument();
      } else {
        ShowToastDialog.toast("Failed to suspend driver");
      }
    } catch (e) {
      log("Error suspending driver: $e");
      ShowToastDialog.toast("Error suspending driver");
    } finally {
      isLoading.value = false;
    }
  }
